        ...
        <main style={s.page}>
            ...
            <div style={s.inputRow}>
                <input type="text" value={pkg} onChange={(e) => setPkg(e.target.value)}
                       placeholder="Package name..." onKeyDown={(e) => e.key === "Enter" && load()} style={{ flex: 1 }} />
                            ...
                            <div style={s.card}>
                                ...
                                <div style={{ marginTop: 8 }}>
                                    {data.keywords?.slice(0, 6).map((k: string) => (
                                        <span key={k} style={s.badge}>{k}</span>
                                    ))}
                                </div>
                            </div>
                            <div style={s.card}>
                                ...
                                <InfoRow k="Downloads (letzter Monat)" v={data.downloads_last_month?.toLocaleString() ?? "—"} />
                                <InfoRow k="Dependencies" v={data.dependencies} />
                                <InfoRow k="Erstellt" v={data.created_at ? new Date(data.created_at).toLocaleDateString("de-DE") : "—"} />
                                <InfoRow k="Aktualisiert" v={data.updated_at ? new Date(data.updated_at).toLocaleDateString("de-DE") : "—"} />
                                {data.homepage && <InfoRow k="Homepage" v={<a href={data.homepage} target="_blank" rel="noreferrer" style={{ color: "var(--color-text-info)" }}>{data.homepage}</a>} />}
                                <InfoRow k="NPM" v={<a href={data.npm_url} target="_blank" rel="noreferrer" style={{ color: "var(--color-text-info)" }}>{data.npm_url}</a>} />
                            </div>
