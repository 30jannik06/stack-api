...
export async function GET(
    ...
    const [summaryRes, gamesRes] = await Promise.all([
        fetch(`${STEAM_API}/ISteamUser/GetPlayerSummaries/v2/?key=${KEY()}&steamids=${steamid}`, { next: { revalidate: 60 } }),
        fetch(`${STEAM_API}/IPlayerService/GetRecentlyPlayedGames/v1/?key=${KEY()}&steamid=${steamid}&count=5`, { next: { revalidate: 60 } }),
    ]);
