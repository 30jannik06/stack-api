...
export default function TwitchPage() {
    ...
    const s: Record<string, React.CSSProperties> = {
        ...
        liveBadge: { fontSize: 11, fontWeight: 500, background: "var(--color-background-danger)", color: "var(--color-text-danger)", borderRadius: 6, padding: "2px 8px", display: "inline-block" },
    };
        ...
        <main style={s.page}>
            ...
            <div style={s.inputRow}>
                <input type="text" value={username} onChange={(e) => setUsername(e.target.value)}
                       placeholder="Username..." onKeyDown={(e) => e.key === "Enter" && load()} style={{ flex: 1 }} />
                        ...
                        <>
                            <div style={s.card}>
                                <div style={{ display: "flex", gap: 12, alignItems: "center" }}>
                                    <img src={data.profile_image_url} style={{ width: 64, height: 64, borderRadius: "50%", flexShrink: 0 }} alt="avatar" />
                                    <div style={{ flex: 1 }}>
                            ...
                            {data.is_live && data.stream && (
                                <div style={s.card}>
                                    ...
                                    <img src={data.stream.thumbnail_url} style={{ width: "100%", borderRadius: 8, marginBottom: 10 }} alt="thumbnail" />
                                    ...
                                    <div style={{ display: "flex", gap: 8, marginTop: 8, flexWrap: "wrap" }}>
                                        {data.stream.tags?.map((t: string) => <span key={t} style={s.badge}>{t}</span>)}
                                    </div>
                                    <div style={{ marginTop: 10 }}>
                                        <InfoRow k="Zuschauer" v={data.stream.viewers.toLocaleString()} />
                                        <InfoRow k="Live seit" v={new Date(data.stream.started_at).toLocaleString("de-DE")} />
                                    </div>
                                </div>
                            )}
